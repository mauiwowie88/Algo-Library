...
//1st attempt: 
const inteval = (arr1, arr2) => {
  // This is our output var
  const output = [];
  // This is to keep track of the last numbers in queue to check overlapping
  const queue = [];
  for (const arr of arr1) {
      const [i,j] = num;
      const [x,y] = arr2;
      // Check if curr arr in arr1 first or second index is between arr2 first or second index
      if (i >= x && i <= y || j >= x && j <= y) {
          // If queue has length
          if (queue.length) {
              // Well check to see if the curr arr highest is > than queues highest; if so swap
              if (queue[1] < j) queue[1] = j;
              continue;
          } else {
              // If queue doesn'tt have length just push array into output
              queue.push(...arr);
              continue;
          }
      }
      // If numbers are not overlapping; check if queue has a length; if so push to output
      if (queue.length) output.push(queue);
      output.push(arr);
  }
  return output;
}
